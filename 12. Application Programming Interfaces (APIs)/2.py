...
request = f"http://api.openweathermap.org/geo/1.0/direct?q={city_name},US&appid={User Token}"
response = requests.get(request).json()

city_lat = response[0]['lat']
city_lon = response[0]['lon']

request2 = f"https://api.openweathermap.org/data/2.5/weather?lat={city_lat}&lon={city_lon}&appid={User Token}&units=metric"
